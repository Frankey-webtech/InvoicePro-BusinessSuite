...
function applyStoredSettingsToSection(sectionId,settings){
    const previousSectionId = currentSectionId;
    const previousPreviewSection = previewSection;
    const previousSettings = {...templateSettings};

    currentSectionId = sectionId;
    previewSection = $(sectionId);
    setCurrentSettings(normalizeTemplateSettings(settings));
    applySettings();

    currentSectionId = previousSectionId;
    previewSection = previousPreviewSection;
    templateSettings = previousSettings;
}

async function loadAllTemplatesFromCloud(){
    const responses = await Promise.all(
        sections.map(async item=>{
            try{
                const response = await Parse.Cloud.run(TEMPLATE_GET_FUNCTION,{section:item.section});
                return {section:item.section,response};
            }catch(error){
                console.error(`Template loading failed for ${item.section}:`,error);
                return {section:item.section,response:null};
            }
        })
    );

    responses.forEach(({section,response})=>{
        const settings = response && response.exists && response.settings
            ? normalizeTemplateSettings(response.settings)
    ...
    responses.forEach(({section})=>{
        ...
        applyStoredSettingsToSection(section,settings);
    });
...
window.addEventListener("DOMContentLoaded",async()=>{
    sections.forEach(item=>sectionSettings[item.section]=cloneDefaults());
    ...
    try{
        await loadAllTemplatesFromCloud();
    }catch(error){
        console.error("Loading saved invoice template failed:",error);
    }
